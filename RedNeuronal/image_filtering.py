        ...
        for image_file in files:
            if image_file.endswith((".png", ".jpg", ".jpeg", ".JPG", ".JPEG")):
                ...
                mask = skimage.morphology.dilation(
                    mask, disk(8)
                )  # Para que no recorte al borde justo
                labeled_img = skimage.measure.label(mask)
                regions = skimage.measure.regionprops(labeled_img)
                region = max(
                    regions, key=lambda r: r.area
                ...
                if (max_row - min_row) > (
                    max_col - min_col
                ):  # me quedo con el mínimo cuadrado posible
                    image_segmented = image[
                        min_row:max_row, min_col : min_col + (max_row - min_row)
                    ]
                else:
                    image_segmented = image[
                        min_row : min_row + (max_col - min_col), min_col:max_col
                    ]
